...
class PublicKey:
    ...
    def save_public_key(self, fileName: str = "public_keys"):
        data = self.key.public_bytes(
            srlz.Encoding.PEM,
            srlz.PublicFormat.SubjectPublicKeyInfo,
        )
        with open(f"keys{fileName}", "ab") as file:
            file.write(data)
...
def load_public_key(fileName: str = "public_key") -> rsa.RSAPrivateKey:
    with open(f"keys/{fileName}", "rb") as key_file:
        public_key = srlz.load_pem_public_key(key_file.read(), None)
        assert isinstance(public_key, rsa.RSAPrivateKey)
        return public_key
