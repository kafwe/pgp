...
def sending_messages(c):
    while True:
        ...
        image_data_base64 = base64.b64encode(image_data).decode("utf-8")  # Convert image data to base64
        image_data_base64_padded = image_data_base64 + '=' * ((4 - len(image_data_base64) % 4) % 4)  # Add padding if necessary (needed for encoding in base64)

        image_data_length = len(image_data_base64_padded)  # length of the image data to pass into packet header

        # Getting caption information
        caption = input("Enter a caption for the image: ")  # Get caption from user
        
        caption_length = len(caption)  # length of the caption to pass into packet header
